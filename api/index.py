...
HALFBLOOD_API_HOST = "https://halfblood.famapp.in"
...
def number_to_vpa():
    ...
    # Try to verify VPA
    vpa_url = f"{HALFBLOOD_API_HOST}/vpa/verifyExt"
    headers = SESSION.headers.copy()
    headers["host"] = "halfblood.famapp.in"
    payload = {"upi_number": str(number)}
    
    try:
        vpa_resp = requests.post(vpa_url, headers=headers, json=payload, timeout=12)
